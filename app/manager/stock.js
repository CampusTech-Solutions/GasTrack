...
window.addEventListener("load", function (e) {
    ...
    card3.render();
    // table.push(card3);
    var card4 = new SettingCard("Ventes", "Nouvelle Vente", "Enregistrez chaque vente ici", "icon-lg mdi mdi-walk text-warning ms-auto");
    card4.render();
    // table.push(card4);
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if (container) {
                    container.innerHTML = '';
                    for (var d of data) {
                        var brand_name = d.brand.name.replace(/ /g, '-');
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if (container) {
                    container.innerHTML = '';
                    for (var d of data) {
                        var brand_name = d.bottle.brand.name.replace(/ /g, '-');
...
function loadPersonalInfo() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            var container = document.getElementById("managerinfo");
            container.innerHTML = '';
    ...
    xhr2.onreadystatechange = function () {
        if (xhr2.readyState === 4 && xhr2.status === 200) {
            ...
            var container2 = document.getElementById("storeinfo");
...
function entriesForm(bottle_id, image, brand, weight) {
    var stockTypes = localStorage.getItem('stockTypes');
    var stocks;
    var container = document.querySelector('#sub-fsc-2');
...
function salesForm(stockbottle_id, image, brand, weight, stockid, stockname, ucp) {
    var container = document.querySelector('#sub-fsc-2');
    brand = brand.replace(/-/g, ' ');
    container.innerHTML = `<h4>BOUTEILLE</h4>
    <div class="image-container" id="">
...
function createStock() {
    ...
    var stock_name = document.getElementById("stock-name");
    var stock_label = document.getElementById("stock-label");
    if (stock_form)
        stock_form.addEventListener("submit", function (event) { event.preventDefault(); });
    if (stock_name)
...
function createEntry(event) {
    ...
    var bottle = document.getElementById('bottle-id');
...
function createSale(event) {
    event.preventDefault();
    var bottle = document.getElementById('stockbottle-id');
    var pvu = document.getElementById('usp');
    var quantity = document.getElementById('quantity');
...
function personalDataUpdate(id) {
    var submit_button = document.querySelector('.info-validate-button button');
